...
export default function Home() {
    ...
    const loadData = async () => {
      try {
        ...
        if (!migrated && typeof window !== "undefined") {
          ...
          if (lsActiveTs)
            await set("tbr-active-timestamp", parseInt(lsActiveTs));
  ...
  return (
    <div className="h-screen relative overflow-hidden p-4 flex flex-col items-center">
      ...
      {/* Top Right Quiz Me Button */}
      <div className="fixed right-0 top-8 z-[60] pr-0">
        <button
          onClick={() => router.push("/question-review")}
          className="sketch-button py-4 w-10 md:w-12 rounded-l-xl rounded-r-none border-r-0 origin-right hover:scale-105 active:scale-95 transition-all duration-300 font-bold flex flex-col items-center justify-center bg-purple-100 hover:bg-purple-200 text-purple-600 shadow-md"
          style={{ fontFamily: "var(--font-gaegu)" }}
          onClickCapture={() => triggerHaptic(10)}
        >
          <span className="[writing-mode:vertical-rl] rotate-180 text-sm md:text-base tracking-widest leading-none flex items-center gap-2 py-2">
            ME
          </span>
        </button>
      </div>
      ...
      <div className="fixed left-0 top-[60%] -translate-y-1/2 z-[60] flex flex-col gap-8 pl-0">
        <button
          ...
          onClickCapture={() => triggerHaptic(10)}
        >
          <span className="[writing-mode:vertical-rl] rotate-180 text-lg md:text-xl tracking-widest leading-none">
            WHEEL
          </span>
        </button>
        <button
        ...
        {deferredPrompt && (
          <button
            ...
            onClickCapture={() => triggerHaptic(10)}
          >
            <span className="[writing-mode:vertical-rl] rotate-180 text-lg md:text-xl tracking-widest leading-none text-blue-600">
              INSTALL
            </span>
          </button>
        )}
      ...
      {/* Main Content */}
      <div
        className={`relative ${activeTab === "library" ? "z-50" : "z-10"} flex flex-col items-center w-full max-w-4xl px-4`}
      >
        {activeTab === "spin" && (
          <WheelTab
            books={books.filter(
              (book) => !bookHistory.some((h) => h.title === book),
            )}
        ...
        <div className="fixed right-0 top-[78%] -translate-y-1/2 z-[60] pr-0 animate-in slide-in-from-right-4 duration-500">
          ...
          >
              ...
              <span className="text-[8px] md:text-[10px] mb-2 [writing-mode:vertical-rl] rotate-180 text-red-400 font-bold tracking-widest">
                {(() => {
                  const days = Math.floor(
                    (now - activeTimestamp) / (1000 * 60 * 60 * 24),
                  );
            ...
            <span className="[writing-mode:vertical-rl] rotate-180 text-lg md:text-xl tracking-widest leading-tight line-clamp-1 max-h-[120px] overflow-hidden">
              {activeBook.length > 12
                ? activeBook.substring(0, 12) + "..."
                : activeBook}
            </span>
          </div>
        </div>
            ...
            <div className="flex flex-col gap-3">
              <button
                onClick={() => {
                  ...
                  const newHistory = bookHistory.map((h) =>
                    h.status === "reading" && h.title === activeBook
                      ? { ...h, status: "finished", endDate: Date.now() }
                      : h,
                  );
              ...
              <button
                onClick={() => {
                  ...
                  const newHistory = bookHistory.map((h) =>
                    h.status === "reading" && h.title === activeBook
                      ? { ...h, status: "paused", pausedDate: Date.now() }
                      : h,
                  );
                  setBookHistory(newHistory);
                  setActiveBook(null);
                ...
                <div className="absolute top-full left-1/2 -translate-x-1/2 mt-2 w-full max-w-[280px] max-h-[200px] overflow-y-auto bg-white border-2 border-[var(--ink)] rounded-lg shadow-lg z-50">
                  <ul className="py-2">
                    {books
                      .filter(
                        (book) =>
                          !bookHistory.some((h) => h.title === book) &&
                          book !== winner,
                      )
                      .map((book, i) => (
                        <li key={i}>
                    ...
                    {books.filter(
                      ...
                      <li className="px-4 py-2 text-gray-400 font-bold text-center text-sm">
          ...
          <div className="sketch-card p-6 w-full max-w-sm text-center">
            ...
            <div className="flex flex-col gap-3">
              <button
                onClick={() => {
                  // Delete Logic
                  const newHistory = bookHistory.filter(
                  ...
                  if (
                    activeBook === itemToDelete.title &&
                    itemToDelete.status === "reading"
                  ) {
                    setActiveBook(null);
                    setActiveTimestamp(null);
                  }
